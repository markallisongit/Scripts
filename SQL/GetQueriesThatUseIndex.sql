SELECT
    ...
    MIN(qrs.last_execution_time) as min_execution_time_PST,
    MAX(qrs.last_execution_time) as last_execution_time_PST,
...
WHERE    
    qsp.query_plan like N'%IX_SubOrderStatusChanges_SystemAdministratorID_DateCreated%' -- change this line
	...
	AND qrs.last_execution_time >= DATEADD(day, -1, GETDATE()) 
GROUP BY 
...
WHERE 
    textplan.query_plan like '%IX_SubOrderStatusChanges_SystemAdministratorID_DateCreated%'
